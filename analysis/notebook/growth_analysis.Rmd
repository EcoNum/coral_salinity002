...
nmle::nlsList(data = gr_visu, gr_divctrl ~ stress_recovery1(nd1, ymin, ths, thr) | cond, subset = gr_visu$cond %in% c("hypersalin", "hyposalin"), control = list(gr_visu$gr_divctrl[gr_visu$cond == "control"]), start = list(ymin = 0, ths = 2, thr = 3)) -> mod3

```

## nlshelper::anova_nlslist()
Description
Applies an F-test to a non-linear regression model that includes a grouping variable (fit with nlsList), comparing it to a model without a grouping variable. This is a convenient way to test whether there is an overall effect of the grouping variable on the non-linear relationshi
...
nmle::nlsList(data = gr_visu, gr_divctrl ~ stress_recovery1(nd1, ymin, ths, thr) | cond, subset = gr_visu$cond %in% c("hypersalin", "hyposalin"), start = list(ymin = 0, ths = 2, thr = 3)) -> nlsfull
summary(nlsfull)
nlshelper::anova_nlslist(nlsfull, nlsreduc)
